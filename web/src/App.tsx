...
export default function App() {
  ...
  const landingContent = (
    <div className="h-[100dvh] bg-[#F5F5F0] flex flex-col items-center justify-center px-4 py-3 font-sans overflow-hidden">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-[420px] text-center space-y-3"
      >
        <div className="space-y-1">
          <h1 className="text-4xl font-black tracking-tighter text-[#141414]">
            言言日本語
          </h1>
          <p className="text-[#5A5A40] text-sm italic serif opacity-80">
            언제 어디서나, 단어 한 마디씩
          </p>
        </div>

        <div className="bg-white p-5 rounded-[28px] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-[#E4E3E0] space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['vocab', 'kanji', 'radical'] as StudyMode[]).map((m) => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
                  className={`py-2.5 rounded-2xl font-black text-xs transition-all ${mode === m ? 'bg-[#141414] text-white shadow-lg' : 'bg-[#F5F5F0] text-[#888] border border-transparent'}`}
                >
                  {m === 'vocab' ? '단어' : m === 'kanji' ? '한자' : '부수'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {mode === 'vocab' ? (
              <div className="grid grid-cols-5 gap-2">
                {['N5', 'N4', 'N3', 'N2', 'N1'].map((lv) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="aspect-square flex items-center justify-center rounded-xl border-2 border-[#E4E3E0] hover:border-[#141414] hover:bg-[#141414] hover:text-white transition-all font-black text-sm disabled:opacity-50"
                  >
                    {lv}
                  </button>
                ))}
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-[#141414] text-white py-3 rounded-2xl font-black text-base active:scale-[0.98] transition-all shadow-xl flex items-center justify-center gap-2"
              >
          ...
          <p className="text-[#888] text-xs italic leading-snug">
            {mode === 'vocab'
          ...
          {isLoading && (
            <div className="pt-2 flex items-center justify-center gap-2 text-[#5A5A40]">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">AI 큐레이션 중...</span>
            </div>
          )}
  ...
  const mainContent = (
    <div className="h-[100dvh] bg-[#F5F5F0] flex flex-col px-3 py-2 font-sans items-center overflow-hidden">
      {/* Header */}
      <header className="w-full max-w-[420px] flex justify-between items-center mb-2">
        <button
          onClick={() => setPhase('landing')}
          className="text-[#5A5A40] flex items-center gap-1.5 hover:text-[#141414] transition-colors"
        >
          <BookOpen size={16} />
          <span className="font-black uppercase tracking-[0.2em] text-[9px]">Menu</span>
        </button>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="bg-white px-2.5 py-1.5 rounded-full border border-[#E4E3E0] flex items-center gap-1.5 shadow-sm"
          >
            <CheckCircle2 size={12} className="text-[#22C55E]" />
            <span className="text-[10px] font-black text-[#141414]">{Object.keys(masteredItems).length}</span>
          </button>
          <div className="bg-white px-2.5 py-1.5 rounded-full border border-[#E4E3E0] flex items-center gap-1.5 shadow-sm">
            <Award size={12} className="text-[#5A5A40]" />
          ...
          <button
            ...
            className="bg-white p-1.5 rounded-full border border-[#E4E3E0] shadow-sm text-[#5A5A40]"
          >
            <Search size={14} />
          </button>
        </div>
      ...
      {/* Main Study Area */}
      <main className="w-full max-w-[420px] flex-1 flex flex-col gap-2 min-h-0">
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            >
              <RefreshCcw className="animate-spin text-[#5A5A40]" size={40} />
              <p className="text-[#5A5A40] font-black italic tracking-tight text-sm">AI 큐레이션 중...</p>
            </motion.div>
          ) : (
            <motion.div
            ...
            >
              {/* Kanji Card */}
              <div className="bg-white rounded-3xl p-4 shadow-[0_10px_30px_rgba(0,0,0,0.05)] border border-[#E4E3E0] flex flex-col items-center justify-center relative overflow-hidden flex-1 min-h-0">
                <div className="absolute top-3 left-4 right-4 flex items-center gap-1 flex-wrap">
                  {Array.from({ length: studyPool.length }).map((_, i) => (
                    <div
                      key={i}
                      className={`h-1 rounded-full transition-all ${
                        i === currentIndex ? 'bg-[#141414] w-6' : 'bg-[#E4E3E0] w-3'
                      }`}
                    />
                  ))}
                </div>
                ...
                <AnimatePresence>
                  {feedback !== 'none' && (
                    ...
                    >
                      {feedback === 'correct' ? (
                        <div className="flex items-center gap-1 bg-[#22C55E] text-white px-2.5 py-1 rounded-full font-black shadow-lg">
                          <CheckCircle2 size={12} />
                          <span className="text-[9px] uppercase tracking-widest">Pass</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1 bg-[#FF4B4B] text-white px-2.5 py-1 rounded-full font-black shadow-lg">
                          <XCircle size={12} />
                          <span className="text-[9px] uppercase tracking-widest">Fail</span>
                        </div>
                      )}
                    </motion.div>
                  )}
                ...
                <div className="text-center flex flex-col items-center justify-center">
                  <h2 className="text-[72px] font-black leading-none text-[#141414] select-none break-all px-2 tracking-tighter">
                    {currentItem?.word}
                  </h2>
                  <div className="mt-3 min-h-[40px]">
                    {feedback !== 'none' && (
                      ...
                      >
                        ...
                        <p className="text-xs font-bold text-[#888] italic">
              ...
              <div className="space-y-2">
                <form onSubmit={handleSubmit} className="relative">
                  <input
                    ...
                    type="text"
                    id="input-answer"
                    placeholder={mode === 'vocab' ? "발음 + 의미 (니치 날)" : mode === 'kanji' ? "훈음 (날 일)" : "부수 입력"}
                    value={userInput}
                    onChange={(e) => setUserInput(e.target.value)}
                    readOnly={feedback !== 'none'}
                    className={`w-full bg-white border-2 border-[#E4E3E0] rounded-2xl pl-4 pr-16 py-3 text-base font-black focus:border-[#141414] focus:outline-none transition-all placeholder:text-[#BBB] placeholder:text-sm placeholder:font-bold ${feedback !== 'none' ? 'cursor-default opacity-80' : 'shadow-md'}`}
                    autoComplete="off"
                  />
                  <div className="absolute right-2 top-1/2 -translate-y-1/2">
                    {feedback === 'none' ? (
                      <button
                        type="submit"
                        id="btn-submit"
                        className="bg-[#141414] text-white p-2 rounded-xl active:scale-90 transition-all shadow-md"
                      >
                      ...
                      <button
                        ...
                        id="btn-next"
                        onClick={handleNext}
                        className="bg-[#141414] text-white px-3 py-2 rounded-xl flex items-center gap-1 font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-md"
                      >
                        NEXT
                ...
                <AnimatePresence>
                  {feedback === 'wrong' && (
                    <motion.div
                      ...
                      className="bg-red-50 border border-red-100 px-3 py-2 rounded-2xl text-red-600 text-center"
                    >
                      <p className="text-[9px] font-black uppercase tracking-widest opacity-50">정답</p>
                      <p className="text-sm font-black">{currentItem.hangulReading} ({currentItem.meaning})</p>
                    </motion.div>
                  )}
                  ...
                  {feedback === 'correct' && (
                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: 'auto' }}
                      className="bg-green-50 border border-green-100 px-3 py-2 rounded-2xl text-green-700"
                    >
                      <p className="text-center text-[11px] font-medium leading-snug opacity-80 italic">"{currentItem.example}"</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              {/* Progress Summary */}
              <div className="flex gap-2">
                <div className="flex-1 bg-white px-3 py-2 rounded-2xl border border-[#E4E3E0] shadow-sm flex items-center justify-between">
                  <p className="text-[9px] font-black text-[#888] uppercase tracking-[0.2em]">SESS</p>
                  <p className="text-base font-black text-[#141414]">{sessionCount}</p>
                </div>
                <div className="flex-1 bg-white px-3 py-2 rounded-2xl border border-[#E4E3E0] shadow-sm flex items-center justify-between">
                  <p className="text-[9px] font-black text-[#888] uppercase tracking-[0.2em]">PROG</p>
                  <p className="text-base font-black text-[#141414]">
                ...
                <button
                  ...
                  onClick={nextSet}
                  disabled={isLoading}
                  className="flex-1 bg-[#5A5A40] text-white px-3 py-2 rounded-2xl font-black uppercase tracking-[0.15em] text-[10px] active:scale-[0.98] transition-all flex items-center justify-center gap-1 disabled:opacity-50 shadow-md"
                >
                  <RefreshCcw size={12} />
                  Next Set
                </button>
